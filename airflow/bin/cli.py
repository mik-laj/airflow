...
class CLIFactory:
    ...
    subparsers = (
        {
            ...
            'name': 'connections',
            'subcommands': SUBCOMMAND_CONNECTIONS,
        }, {
            'help': "List/Create/Delete/Update users",
            'name': 'users',
            'subcommands': SUBCOMMAND_USERS,
        }, {
            'help': 'Create/List roles',
            'name': 'roles',
            'subcommands': SUBCOMMANDS_ROLES,
        }, {
            'name': 'sync_perm',
    ...
    def get_parser(self):
        ...
        for sub in sorted(self.subparsers, key=lambda d: d['name']):
            self._add_command(subparsers, sub)
        return parser

    def _add_command(self, subparsers, sub):
        ...
        if 'subcommands' in sub:
            self._add_subcommand(sub, sub_proc)
        elif 'func' in sub:
            self._add_action(sub, sub_proc)
        else:
            raise ValueError("Invalid CLI structure")

    def _add_subcommand(self, sub, sub_proc):
        sub_subparsers = sub_proc.add_subparsers(dest='subcommand')
        sub_subparsers.required = True
        for command in sub['subcommands']:
            self._add_command(sub_subparsers, command)

    def _add_action(self, sub, sub_proc):
        for arg in sub['args']:
            arg = self.args[arg]
            kwargs = {
                f: v
...
class CLIDagFactory(CLIFactory):
    subparsers = (
        *(cmd for cmd in deepcopy(SUBCOMMANDS_DAG) if cmd['name'] != 'list'),
        {
            'help': 'List and manage tasks',
            'name': 'tasks',
            'subcommands': deepcopy(SUBCOMMANDS_TASKS),
        }
    )

    def _add_action(self, sub, sub_proc):
        sub['args'] = {v for v in sub['args'] if v != 'dag_id'}

        super()._add_action(sub, sub_proc)
...
def get_dag_parser():
    """Calls static method inside factory which creates argument parser"""
    return CLIDagFactory().get_parser()
